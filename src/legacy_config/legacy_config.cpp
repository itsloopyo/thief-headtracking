...
constexpr float kDefaultCollisionRelease = 0.9f;
...
// Returns a refusal on a port outside the bindable range, which is the one config error
    ...
    if (!portText.empty()) {
        ...
        if (end == begin) {
            Log::Line("ERROR: INI [General] Port value '%s' is not a number",
                      portText.c_str());
            return {ReadStatus::Refused, "[General] Port=" + portText + " is not a number"};
        }
    }
